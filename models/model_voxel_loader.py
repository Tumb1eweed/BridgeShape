...
from models.p2pb_vqvae import P2PB_VQVAE
from models.models_vq.model_utils import load_vqvae
from models.p2p_3d import VQVAE_P2P_Model
...
def load_VQVAE_P2P_model(cfg: Dict) -> torch.nn.Module:
    model=VQVAE_P2P_Model(cfg=cfg)
    logger.info(
        f"Generated model with following number of params (M): {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}"
    )
    return model

def load_vqvae_voxel_diffusion(cfg: Dict) -> tuple:
    vqvae = load_vqvae(cfg.vqvae.vq_conf,cfg.vqvae.vq_ckpt,cfg.local_rank)
    backbone = load_VQVAE_P2P_model(cfg).to(cfg.local_rank)
    model = P2PB_VQVAE(cfg=cfg, model=backbone,vqvae=vqvae).to(cfg.local_rank)
    gpu = cfg.local_rank
        ...
        if not cfg.restart:
            ...
            try:
                ...
                if cfg.distribution_type in ["multi", "single"]:
                    ...
                    model_dict = extract_from_state_dict(model_state, "model.module.")
                model.model.module.load_state_dict(model_dict)
