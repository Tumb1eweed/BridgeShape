...
import numpy as np
from .resunet3d import ControlledUNet
...
class Encoder3D(nn.Module):
    def __init__(self, *, ch=32, ch_mult=(1,2), num_res_blocks=1,
                 attn_resolutions=[], dropout=0.0, resamp_with_conv=True, in_channels=2,
                 resolution=32, z_channels=3, double_z=False,
                 activ='gelu',
                 **ignore_kwargs):
        super().__init__()
        self.ch = ch #64
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult) #ch_mult: [1,2]#,4]
...
class Decoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.ch = ch  #64
        self.temb_ch = 0
        self.num_resolutions = len(ch_mult) #[1,2,4] 3
        self.num_res_blocks = num_res_blocks #1
        self.resolution = resolution  #64
        self.in_channels = in_channels #1
        ...
        curr_res = resolution // 2**(self.num_resolutions-1) #64 ->(64/2**2) = 16
        self.z_shape = (1,z_channels,curr_res,curr_res, curr_res)
        print("Working with z of shape {} = {} dimensions.".format(
            self.z_shape, np.prod(self.z_shape)))

        # z to block_in
        self.conv_in = torch.nn.Conv3d(z_channels, #3 -> 64*4
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        ...
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out = torch.nn.Conv3d(block_in, #64*4
                                        out_ch, #1
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
...
class VQVAE_P2P_Model(nn.Module):
    def __init__(self,cfg):
        super(VQVAE_P2P_Model, self).__init__()

        self.encoder = Encoder3D(ch=32, ch_mult=[1,2], num_res_blocks=1,attn_resolutions=[], dropout=0.0, resamp_with_conv=True, in_channels=2,resolution=32, z_channels=3, double_z=False,activ='gelu')
        self.unet = ControlledUNet(
            in_channels=cfg.net.in_channels, #3
            model_channels=cfg.net.model_channels, #64
            out_channels=3,  # 1
            num_res_blocks=cfg.net.num_res_blocks,  # 3
            channel_mult=cfg.net.channel_mult,  # (1, 2, 2, 2)
            dropout=0,
            dims=3,
            activation=None
        )

    def forward(self, x, timesteps=None, control=None, **kwargs):
        x = self.encoder1(x)

        x = self.unet(x, timesteps=timesteps, control=control, **kwargs)
