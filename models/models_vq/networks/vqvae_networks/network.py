...
from models.models_vq.networks.vqvae_networks.vqvae_modules import Encoder3D, Decoder3D
...
class VQVAE(nn.Module):
    def __init__(self,
        ...
        print("Working with Codebook of length {} .".format(self.n_embed))
        self.quantize = VectorQuantizer(n_embed, embed_dim, beta=1.0,
                                        remap=remap, sane_index_shape=sane_index_shape, legacy=False)
        self.quant_conv = torch.nn.Conv3d(ddconfig["z_channels"], embed_dim, 1)      #3 -> 3
        self.post_quant_conv = torch.nn.Conv3d(embed_dim, ddconfig["z_channels"], 1) #3 -> 3
    ...
    def encode(self, x):
        ...
        h = self.encoder(x)
        # print("after_encoder:",h.shape)
        h = self.quant_conv(h)
        # print("after_quant_conv:", h.shape)
    ...
    def decode(self, quant):
        # print("decoder_quant:",quant.shape)
        quant = self.post_quant_conv(quant)
        # print("after_decoder_quant:",quant.shape)
        dec = self.decoder(quant)
        # print("after_decoder:",dec.shape)
        return dec
