...
class P2PB_VQVAE(DiffusionModel):
    def __init__(self, cfg: Dict, model: torch.nn.Module, vqvae=None):
        super().__init__()
        # setup config
        ...
        self.add_x1_noise_w = cfg.diffusion.add_x1_noise_w if "add_x1_noise_w" in cfg.diffusion else False
    ...
    def ddpm_sampling(
        ...
        def pred_x0_fn(xt, step, x1, x_cond=None):
            ...
            if use_ema and self.ema is not None:
                ...
                out = self.model.module.unet(xt, noise_levels)
            if self.objective == "pred_noise":
    ...
    def latent_sample(
        ...
        x1_latent = self.model.module.encoder(x1)  # [B,2,32,32,32]->[B,3,16,16,16]
        xs, x0s = self.ddpm_sampling(
            x1=x1_latent,
            x_cond=x_cond,
            clip_denoise=clip,
        ...
        latent_pred_x0 = data['x_pred']  # [B,3,16,16,16]
        pred_x0 = self.vqvae.module.decode_no_quant(latent_pred_x0)  # [B,1,32,32,32]
        return data, pred_x0
    ...
    def forward(
        ...
        with torch.no_grad():
            if dino is not None:
                x0_latent = self.vqvae.module(x0, dino=dino, forward_no_quant=True, encode_only=True)  # [B,1,64,64,64]->[B,3,16,16,16]
            else:
                x0_latent = self.vqvae.module(x0, forward_no_quant=True,encode_only=True)  # [B,1,64,64,64]->[B,3,16,16,16]
        x1_latent = self.model.module.encoder(x1)  # [B,2,32,32,32]->[B,3,16,16,16]
        steps = torch.randint(0, self.timesteps, (x0_latent.shape[0],)).to(self.device)
        if self.add_x1_noise:
            x1_latent = x1_latent + torch.randn_like(x1_latent)
        if self.add_x1_noise_w:
            x1_latent = x1_latent + 0.1*torch.randn_like(x1_latent)
        xt_latent = self.q_sample(steps, x0_latent, x1_latent)  # [B,3,16,16,16]
        gt_latent = self.compute_gt(steps, x0_latent, xt_latent)  # [B,3,16,16,16]

        noise_levels = self.noise_levels[steps]
        pred_latent = self.model.module.unet(xt_latent, timesteps=noise_levels)  # [B,3,16,16,16]
        loss = self.calculate_loss(pred_latent, gt_latent)  # MSE [B,3,16,16,16]<->[B,3,16,16,16]
        loss = loss.mean()
