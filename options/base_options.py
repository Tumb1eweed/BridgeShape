...
class BaseOptions():
    ...
    def initialize(self):
        ...
        # dataset stuff
        self.parser.add_argument('--data_root', type=str, default=None, help='root dir for data. if None, specify by `hostname` in configs/paths.py')
        ...
        self.parser.add_argument('--per_class', type=bool, default=False, help='per_class')
        self.parser.add_argument('--class_id', type=str, default="", help='class_id')
        self.parser.add_argument('--suffix', type=str, default=".pth", help='suffix')
        self.parser.add_argument('--log_df', type=bool, default=False, help='log_df')
        self.parser.add_argument('--representation', type=str, default="tsdf", help='representation')
    ...
    def parse_and_setup(self):
        ...
        if get_rank() == 0:
            ...
            # make experiment dir
            if self.opt.isTrain or self.opt.isTrain==False:
                expr_dir = os.path.join(self.opt.logs_dir, self.opt.name)
                utils.utils_vq.util.mkdirs(expr_dir)
